...
statement returns[Statement ast]
	...
	| 'if' '(' expression ')' '{' tr=statements '}' 'else' '{' fs=statements '}'			{ $ast = new Ifelse($expression.ast, $tr.list, $fs.list); }
	| 'if' '(' expression ')' '{' tr=statements '}'											{ $ast = new If($expression.ast, $tr.list); }
    ;
